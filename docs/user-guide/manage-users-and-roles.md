...
the one page in PatchPilot gated by route, not just by hidden actions. See
...
3. Toggle **write access** for an engineer separately from their role —
   ...
   Administrator in the home tenant. PatchPilot can request the change but
...
5. To check what a *specific* engineer can currently do — rather than what
   ...
   which verifies PatchPilot's own role, direct Entra roles, and GDAP roles
